...
void main(){
  ...
  // null이 나올 때 에러로 연결되므로 이런 경우에
  // 다트는 필수적으로 널세이프티 처리를 강제한다!
  // 물음표(?) 하나는 null일 경우 "null"문자열 출력 대체
  // 물음표 두개(??)는 null 발생 시 문자열 대체한 경우
  // 커스터마이징 대체 문자열을 출력하게 해준다!

  // 익명함수를 만들어서 특정 함수에 값으로 전달한다!
...
String actorList(List list){
  ...
  // for문 : for(시;한;증){코드}
  // 리스트를 순회하는 for문
  // -> for(변수 in 리스트형 변수){코드}
  for(var x in list){ // x - list의 개별 값 순회
    temp += '🍰$x';
  } /////////////////// for ////////////////////////
...
// 4. 익명함수를 함수 호출 시 전달하여 실행하게 하는 함수
void japanShip(Function bomb){ // bomb - 전달익명함수
  showTxt('나는 일본 배야! 각오들 해~~');
  // 전달된 함수 실행
  bomb();
} ////////////// japanShip 함수 //////////////////////////
